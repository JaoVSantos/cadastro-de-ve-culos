...
if (!empty($pagina)) {
    ...
    $query_veiculos = "SELECT id, nome, marca, ano, Valor_venda FROM cadastro ORDER BY id DESC LIMIT $inicio, $qnt_result_pg";
    ...
    while ($row_veiculo = $result_veiculos->fetch(PDO::FETCH_ASSOC)) {
        ...
        $dados .= "<tr>
                    ...
                    <td>$Valor_venda</td>
                    <td>
                        <button id='$id' class='btn btn-outline-primary btn-sm' onclick='visCarros($id)'>Visualizar</button>

                        <button id='$id' class='btn btn-outline-warning btn-sm' onclick='editCarrosDados($id)'>Editar</button>

                        <button id='$id' class='btn btn-outline-danger btn-sm' onclick='apagarCarrosDados($id)'>Apagar</button>
                    </td>
                </tr>";
    }
    ...
    $dados .= "<li class='page-item'><a href='#' class='page-link' onclick='listarCarros(1)'>Primeira</a></li>";
